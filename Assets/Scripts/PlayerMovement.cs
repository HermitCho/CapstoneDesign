...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void Rotation()
    {
        ...
        float mappedValue = Mathf.InverseLerp(-parentSize.y / 10f + elementSize.y / 2, parentSize.y / 2 - elementSize.y / 2, currentUIPosition.y);  // Y값을 0에서 1 사이로 변환
        mappedValue = Mathf.Clamp(mappedValue, 0.50f, 0.9f);  // 0.75 ~ 1 사이로 제한
