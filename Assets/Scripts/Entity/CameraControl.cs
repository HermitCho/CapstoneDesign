...
{
    private Transform player; //플레이어 캐릭터
    ...
    // 플레이어 설정을 위한 public 메서드
    public void SetPlayer(Transform newPlayer)
    {
        player = newPlayer;

        // 씬에서 CinemachineVirtualCamera 찾기
        CinemachineVirtualCamera virtualCamera = FindObjectOfType<CinemachineVirtualCamera>();
        if (virtualCamera != null)
        {
            // 선택된 플레이어를 Follow 타겟으로 설정
            virtualCamera.Follow = player;
            Debug.Log($"CinemachineVirtualCamera의 Follow 타겟이 {player.name}으로 설정되었습니다.");
        }
        else
        {
            Debug.LogError("씬에서 CinemachineVirtualCamera를 찾을 수 없습니다!");
        }
    }
